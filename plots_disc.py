...
fig = plt.figure(figsize=(6.8, 2.8), dpi=300)
...
axes[0].set_ylabel('Isochromatic Image', rotation=0, labelpad=20, ha='right')
axes[len(filenames)].set_ylabel('Predicted Stress Map', rotation=0, labelpad=20, ha='right')

for i, filename in enumerate(filenames):
    axes[i].set_title(format((i + 1)*0.981, '.2f'))

fig.text(0.01, 0.917, 'Compressive Force (kN)', ha='left', va='center')
...
for i, filename in enumerate(filenames):
    ...
    max_stress = 151
    res = axes[i + 4].imshow(predict_img.data.cpu().numpy().squeeze(0) / max_stress, cmap='jet', vmin=0, vmax=1)
